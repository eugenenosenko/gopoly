...
// LookupImports will fetch required imports for the required code.Data, without duplicates and
// returns it in a proper string format.
func LookupImports(d code.Data) string {
	...
	for _, t := range d.Types() {
		for _, v := range maps.Values(t.Variants()) {
			for _, field := range v.Fields() {
				if i, ok := m[field.Interface().Pkg().String()]; ok {
